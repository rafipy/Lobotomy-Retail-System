...
def update_user_profile(
    user_id: int, update_data: UpdateUserProfile, db=Depends(get_db)
):
    """
...
def update_username(user_id: int, username_data: UpdateUsername, db=Depends(get_db)):
    """
    Update user's username

    WHAT IT DOES:
    1. Checks if the new username is already taken
    2. Updates the username in the database
    """
    cursor = db["cursor"]
    ...
    cursor.execute(
        ...
        (username_data.new_username, user_id),
    )
    if cursor.fetchone():
